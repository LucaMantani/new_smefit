...
class _ZprimeBlock:
    ...
    def __init__(self, coefficients, rge_dict=None, model=None, **_):
        ...
        probe = uv.smeft_wcs({**self._defaults, "g": 0.2, "tsb": 0.1, "tdb": 0.1,
                              "tds": 0.1}, self._charges, self._qq1, np)
...
class _FlavourBlock(_ZprimeBlock):
    ...
    def __init__(self, coefficients, rge_dict=None, model=None, tables=None,
                 penalty=1e4, dg_tol=2.0, **kw):
        ...
        self._jTd = None if self.S.jms_T_dlnv is None else jnp.asarray(self.S.jms_T_dlnv)
    ...
    def _warsaw_to_jms(self, tables_path):
        ...
        import wilson
        m = self.S.df2.m
        ckm = {"Vus": m["Vus"], "Vcb": m["Vcb"], "Vub": m["Vub"], "gamma": m["gamma_sm"]}
        out = list(self.S.jms_names)
        key = hashlib.md5(repr((self.warsaw, self.M, out, wilson.__version__, "integrate",
                                sorted(ckm.items()))).encode()).hexdigest()[:16]
        ...
        for j, (n, part) in enumerate((n, p) for n in self.warsaw for p in (1.0, 1j)):
            w = wilson.Wilson({n: probe * part}, scale=self.M, eft="SMEFT", basis="Warsaw")
    ...
    def compute_chi2(self, values):
        ...
        if self._jTd is not None:
            dlnv = d2.dlnv(nockm)
            if dlnv is not None:
                x = x + jnp.tensordot(dlnv, design.poly_eval(self._jTd, dg, jnp) @ y,
                                      axes=(0, 0))
...
class _EWBlock(_ZprimeBlock):
    ...
    def __init__(self, coefficients, rge_dict=None, model=None, tables=None, **kw):
        super().__init__(coefficients, rge_dict, model, **kw)
        self.E = ew.EWSurrogate(_abs(tables, os.path.join(TABLES, self.SUPPORT, "tables.pkl")),
                                blocks=(self.BLOCK,))
        if abs(self.E.coords.scale - self.M) > 1e-6 * self.M:
            raise ValueError("the EW tables are built at %g GeV, the model card has "
        ...
        self._sc = jnp.asarray([(1.0 if n in pos else 0.0) for n in names]) \
