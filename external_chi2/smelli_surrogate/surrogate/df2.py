...
OBS_MESON = {'DeltaM_s': 'Bs', 'S_psiphi': 'Bs', 'S_psiK': 'B0',
             'eps_K': 'K0', 'x12Im_D': 'D0'}
...
def mesons_for(coords):
    ...
    ``phis_const`` instead.
    """
    return [m for m, sector in MESON_SECTOR.items()
...
def gamma_from_fac(f1, f2, f3, f4, xp=np):
    ...
    """
    sqrt = xp.sqrt
    arg = (-((-1 + f3)*(f3 - f3*(f2 + f3) - f2*f4 + f1*(f2 + f3*(-1 + f2 + f3)*f4)))
...
def build(gl, coords, verbose=True):
    ...
    for meson in mesons_for(coords):
        ...
        mesons[meson] = dict(sector=sector, idx=np.array(idx, dtype=int),
                             wcs=wcs, units=np.array(units, dtype=float))
    ...
    def node_row(dg, ckm_shift=None):
        ...
        for meson, m in mesons.items():
            ...
            for wc, unit in zip(m['wcs'], m['units']):
                ...
                k.append((M1 - M0) / unit)
            out[meson] = dict(M12sm=M0, G12=G0,
                              k=np.array(k, dtype=complex) if k
        ...
        for p in DLNV:
            ...
            for meson in mesons:
                for key in ('M12sm', 'G12', 'k'):
                    ...
                    drv[meson][key].append(np.where(np.abs(q0) > 0, d / np.where(
                        np.abs(q0) > 0, q0, 1.0), 0.0))
        for meson in mesons:
            rows_v[meson].append({key: np.array(drv[meson][key])
                                  for key in ('M12sm', 'G12', 'k')})
        if verbose:
            print('  df2 node dg=%+.4f  ' % dg
                  + '  '.join('|M12_%s|=%.4e' % (m, abs(rows[m][-1]['M12sm']))
                              for m in mesons), flush=True)

    tab = {}
    for meson, m in mesons.items():
        block = dict(m)
        for key in ('M12sm', 'G12', 'k'):
            block[key] = design.interp_nodes(nodes, np.array([r[key] for r in rows[meson]]))
    ...
    for obs, (meson, etaCP) in S_OBS.items():
        ...
        drv = [[(_ratio(dg, {p: ckm_sm[p] * (1 + EPS_LNV)})
                 - _ratio(dg, {p: ckm_sm[p] * (1 - EPS_LNV)}))
                / (2 * EPS_LNV) / _ratio(dg) for p in DLNV] for dg in nodes]
    ...
    if verbose:
        print('  df2 CKM inputs: exp/nockm vs ckm_fac(SM), relative: %s'
              % np.array2string(rel, precision=2), flush=True)
    tab['meta'] = dict(gamma_sm=g_sm, Vus=ckm['Vus'], Vcb=ckm['Vcb'], Vub=ckm['Vub'],
                       f1=fac_sm[0], f2=fac_sm[1], f3=fac_sm[2],
                       exp_cen4=exp_cen[3], exp_cen123=np.array(exp_cen[:3]),
                       nockm_sm=nockm_sm[:3],
                       nodes=nodes, mesons=list(mesons))
    return tab
