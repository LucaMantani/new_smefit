        ...
        for mname in L[fn].measurement_likelihood.get_measurements:
            ...
            for constraint, pars in M._constraints:
                keep = [
                    ...
                    if str(p) in pos and M._parameters.get(p) == (k, constraint)
                ]
                if not keep:
                ...
                if isinstance(constraint, MultivariateNormalDistribution):
                    ...
                    raise NotImplementedError(
                        ...
                        "are handled beyond 1D"
                        % (pars, mname, len(pars), type(constraint).__name__)
                    )
                    ...
                    terms.append(
                        dict(
                            ...
                            sigma=(
                                float(constraint.left_deviation),
                    ...
                    terms.append(
                        dict(
                            ...
                            kind="normal",
                            mu=np.atleast_1d(constraint.central_value).astype(float),
                            sigma=np.atleast_1d(constraint.standard_deviation).astype(
                                float
                            ),
                        )
                    )
...
def pack(terms):
    ...
    for t in terms:
        if t["kind"] == "normal":
            ...
            rest.append(t)
    return dict(
        idx=np.concatenate(idx) if idx else np.zeros(0, dtype=int),
...
def chi2(pred, terms, xp=np):
    ...
    for t in rest:
        ...
        if t["kind"] == "multivariate":
            ...
            p_ = xp.interp(
                pred[np.asarray(t["idx"])], xp.asarray(t["x"]), xp.asarray(t["y"])
            )
...
def make_w(wc_dict, scale=MATCHING_SCALE):
    ...
    w = wilson.Wilson(
        dict(wc_dict) or {"ll_1111": 0.0}, scale=scale, eft="SMEFT", basis="Warsaw"
    )
...
def check(blocks=BLOCKS, wc=None):
    ...
    *offset is the same at every point*: measured -842.737526 at three points
    ...
    for fn in blocks:
        d = dict(
            zip(
                [str(o) for o in L[fn].observables],
                [
                    pred[i]
...
class EWCoords:
    ...
    def to_x(self, wc):
        ...
        return np.array(
            [complex(wc.get(n, 0.0)).real / u for n, u in zip(self.names, self.units)]
        )
...
def build(coords_name="EW1", workers=6, n_random=8, out=None, verbose=True):
    ...
    if verbose:
        print(
            "%s: %d coordinates, %d observables, %d design points (%d exact)"
            % (co.name, len(co), n, len(X), n_exact),
            flush=True,
        )
    ...
    with ctx.Pool(workers, initializer=_build_init, initargs=(coords_name,)) as pool:
        ...
        for i, y in enumerate(pool.imap(_build_task, [row for row in X])):
            ...
            if verbose and (i % 50 == 0 or i == len(X) - 1):
                print(
                    "  %4d/%d  (%.1f min)" % (i + 1, len(X), (time.time() - t0) / 60),
                    flush=True,
                )
    Y = np.array(Y)
    C = np.linalg.lstsq(design.quad_features(X), Y, rcond=None)[0]
    res = np.max(np.abs(design.quad_features(X) @ C - Y), axis=0) / (
        np.abs(Y).max(axis=0) + 1e-300
    )
    if verbose:
        k = int(np.argmax(res))
        print(
            "max relative design residual %.2e  (worst %s)"
            % (res.max(), obs_name(obs[k])),
            flush=True,
        )
...
class EWSurrogate:
    ...
    def unsupported(self, wc, rtol=0.0):
        ...
        return sorted(
            k for k, v in wc.items() if k not in keep and abs(complex(v)) > rtol
        )
...
def validate(coords_name="EW1", n=20, seed=11, box=None, tables=None, verbose=True):
    ...
    for i in range(n + 1):
        ...
        if verbose:
            print(
                "%-6d %12.5f %12.5f %12.2e   %8.2e  %s"
                % (i, c_ref, c_sur, c_sur - c_ref, abs(r[j]), obs_name(obs[j]))
            )
    d2 = np.array(d2)
    if verbose:
        print(
            "\n|Delta chi2|: max %.2e, rms %.2e"
            % (np.abs(d2).max(), np.sqrt((d2**2).mean()))
        )
    return d2
...
def main(argv=None):
    ...
    if a.add_block_sizes:
        ...
        with open(path, "wb") as fh:
            pickle.dump(t, fh)
        print("{}: block sizes {}".format(path, sizes))
    elif a.validate:
