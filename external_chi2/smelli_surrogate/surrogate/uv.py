...
def vdl_matrix(p, xp=np):
    ...
    rot_13 = M([[c13, zero, s13 * e(-p['delta'])],
                [zero, one, zero],
                [-s13 * e(p['delta']), zero, c13]])
    rot_12 = M([[c12, s12, zero], [-s12, c12, zero], [zero, zero, one]])
    phase_L = M([[one, zero, zero], [zero, e(p['phi1']), zero], [zero, zero, e(p['phi2'])]])
    phase_R = M([[e(-p['phi3']), zero, zero],
                 [zero, e(-p['phi4']), zero],
                 [zero, zero, e(-p['phi5'])]])
...
def load_model(path):
    ...
    defaults = {'M': float(m['Zprime_properties']['M']),
                'g': float(m['Zprime_properties']['g'])}
    for block, keys in (('quark_mixing_angles', ('tsb', 'tdb', 'tds')),
...
def check(model_path, n=8, seed=0):
    ...
    for t in range(n):
        ...
        card['complex_phases'] = {k: p[k] for k in
                                  ('delta', 'phi1', 'phi2', 'phi3', 'phi4', 'phi5')}
