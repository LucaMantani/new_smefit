...
class smefitConfig(Config):
    ...
    @element_of("reference_points")
    def parse_reference_point(self, point: Mapping) -> ReferencePoint:
        """Read one point of coefficient space to mark on a figure.

        ``reference_points`` is the list form of this key, generated by
        ``element_of``. The figures that draw them keep marking the SM point
        on their own, so these are additions to it:
        ...
        ``values`` names only the coefficients that move: every other one keeps
        its ``baseline_value``, so an entry with no ``values`` at all marks the
        ...
        if not isinstance(label, str):
            raise ConfigError(
                "Each reference_points entry requires a string 'label', the "
