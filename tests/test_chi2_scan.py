...
def test_run_chi2_scan_writes_json(tmp_path):
    from smefit.utils_actions import run_chi2_scan

    scans = [
        {"OpA": {"points": [-1.0, 0.0, 1.0], "chi2": [1.0, 0.0, 1.0]}},
        {"OpB": {"points": [-1.0, 0.0, 1.0], "chi2": [4.0, 0.0, 4.0]}},
    ]
    run_chi2_scan(scans, tmp_path)
    assert (tmp_path / "chi2_scan.json").exists()


def test_run_chi2_scan_merges_results(tmp_path):
    from smefit.utils_actions import run_chi2_scan

    scans = [
        {"OpA": {"points": [0.0], "chi2": [1.0]}},
        {"OpB": {"points": [0.0], "chi2": [2.0]}},
    ]
    run_chi2_scan(scans, tmp_path)
    data = json.loads((tmp_path / "chi2_scan.json").read_text())
    assert data["OpA"]["chi2"] == [1.0]
    assert data["OpB"]["chi2"] == [2.0]


def test_run_chi2_scan_valid_json(tmp_path):
    from smefit.utils_actions import run_chi2_scan

    scans = [{"OpA": {"points": [-1.0, 0.0, 1.0], "chi2": [1.0, 0.0, 1.0]}}]
    run_chi2_scan(scans, tmp_path)
    data = json.loads((tmp_path / "chi2_scan.json").read_text())
