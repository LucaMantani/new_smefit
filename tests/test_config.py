...
# ---------------------------------------------------------------------------
# parse_reference_point / the reference_points key it generates
# ---------------------------------------------------------------------------


def test_parse_reference_points_reads_an_entry(cfg):
    points = cfg.parse_reference_points(
        [{"label": "$A$", "values": {"OtG": 0.1}, "marker": "x", "color": "firebrick"}]
    )

    assert len(points) == 1
    assert points[0] == ReferencePoint(
        label="$A$", values={"OtG": 0.1}, marker="x", color="firebrick"
    )


def test_parse_reference_points_defaults_to_the_baseline_everywhere(cfg):
    """An entry with no values marks the same point as the SM marker: the
    coordinates are filled in from the fits' baselines when it is drawn."""
    points = cfg.parse_reference_points([{"label": "$A$"}])

    assert points[0].values == {}
    assert points[0].marker is None and points[0].color is None


def test_parse_reference_points_is_a_namespace_list(cfg):
    """element_of gives the plural, so a report can also expand over it."""
    points = cfg.parse_reference_points([{"label": "$A$"}])

    assert isinstance(points, NSList)
    assert points.nskey == "reference_point"


def test_parse_reference_points_requires_a_label(cfg):
    """Every marker needs a legend entry, or the figure cannot say what it is."""
    with pytest.raises(ConfigError, match="requires a string 'label'"):
        cfg.parse_reference_points([{"values": {"OtG": 0.1}}])


def test_parse_reference_points_rejects_a_non_numeric_value(cfg):
    with pytest.raises(ConfigError, match="must be a number"):
        cfg.parse_reference_points([{"label": "$A$", "values": {"OtG": "0.1"}}])


def test_parse_reference_points_rejects_values_that_are_not_a_mapping(cfg):
    with pytest.raises(ConfigError, match="must be a mapping"):
        cfg.parse_reference_points([{"label": "$A$", "values": ["OtG", 0.1]}])


def test_parse_reference_points_converts_integers(cfg):
    """YAML makes `OtG: 1` an int; matplotlib and the axis limits want floats."""
    points = cfg.parse_reference_points([{"label": "$A$", "values": {"OtG": 1}}])

    assert isinstance(points[0].values["OtG"], float)


def test_parse_reference_points_unknown_key_warns(cfg, caplog):
    """A misspelt `valeus:` would otherwise draw the point at the baseline in
    silence."""
    with caplog.at_level(logging.WARNING):
        points = cfg.parse_reference_points([{"label": "$A$", "valeus": {"OtG": 1.0}}])
